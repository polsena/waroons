...
class slide extends MY_Controller {
    ...
    public $page_title = 'จัดการภาพสไลด์หน้าเว็บ';
    ...
    function index() {
        ...
        $data['content'] = 'admin/slide/slide_frm_v';
    ...
    function news() {
        ...
        $data['ckeditor'] = $this->script_m->get_ckeditor('detail');
        $this->db->order_by('id');
        $data['slide'] = $this->db->get('tbslide')->result_array();
    ...
    function edit($id, $success = '0') {
        ...
        $this->db->where('id', $id);
        $row = $this->db->get('tbslide')->row_array();
        $data['row'] = $row;
        if ($success == '1') {
    ...
    function save() {
        ...
        if (isset($_FILES['filename'])) {
            ...
            // upload here
            if ($name != "") {
                if (move_uploaded_file($file_temp, "assets/upload/file/" . $file_name)) {
                    $data['filename'] = $name;
                    $data['filetmp'] = $file_name;
                    //        print_r($data) ;
                    if ($id == '') {
                        $this->db->insert('tbslide', $data);
                        $id = $this->db->insert_id();
                    } else {
                        $this->db->where('id', $id);
                        $this->db->update('tbslide', $data);
                    }
                }
            }
        }
...
//            $config['max_height'] = '768';
        ...
        if ($id == '') {
            $this->db->insert('tbslide', $file);
            $id = $this->db->insert_id();
        } else {
            $this->db->where('id', $id);
            $this->db->update('tbslide', $file);
        }
    ...
    function ajax_get_grid($offset = 0) {
        ...
        $array = $this->slide_m->get_all($offset);
        $data['query'] = $array['query'];
        $data['pagination'] = $array['pagination'];
        $data['slide_m'] = $this->slide_m;
        $data['select_m'] = $this->select_m;
        $this->load->view('admin/slide/slide_grid_v', $data);
    }
    ...
    function ajax_delete() {
        ...
        $row = $this->db->get('tbslide')->row_array();
        if (count($row) > 0) {
            $full_path = $this->myfile->GetPhysicalFromURL() . 'assets/upload/slider/';
            $file_path = $this->myfile->GetPhysicalFromURL() . 'assets/upload/file/';
